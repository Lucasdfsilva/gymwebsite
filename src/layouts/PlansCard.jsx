...
const PlansCard = (props) => {
  const features = [
    "Sessões diárias gratuitas personal.",
    "Acesso às instalações da academia.",
    "Acesso a todos os quartos.",
    "Uso de equipamentos ginástica.",
  ];

  const renderCircle = (index) => {
    if (props.title === "Bronze") {
      return index === 0 ? (
        <AiFillCheckCircle className="mr-2 mt-1 text-green-500" size={24} />
      ) : (
        <AiFillCloseCircle className="mr-2 mt-1 text-red-500" size={25} />
      );
    } else if (props.title === "Ouro") {
  ...
  return (
    <div className="flex flex-col bg-gray-950 w-full md:w-1/3 p-8 rounded-xl shadow-lg">
      ...
      <div className="flex items-center justify-center">
        ...
        <h3 className="font-semibold text-lg text-white">R$ {props.price}</h3>
      </div>
